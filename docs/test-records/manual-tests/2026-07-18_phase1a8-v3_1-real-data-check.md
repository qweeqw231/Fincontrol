...
- 提交：`0107b78`
- 推送：`+ de6b1c5...0107b78 main -> main (forced update)`
- 远端：`https://github.com/qweeqw231/Fincontrol.git`

包含 18 个文件、395+/200-：

- docs/phase-0/db-schema.sql（+2 列 + 决策 8 占位）
- docs/phase-0/decisions.md（v3 + v3.1 修正）
- docs/phase-1/checklists/phase-1a.md（1a.8.7 ✅）
- docs/test-records/manual-tests/2026-07-18_phase1a8-v2-real-data-check.md（v3.1 升级）
- docs/test-records/manual-tests/2026-07-18_phase1a8-v2-acceptance-plan.md（v3.1 引用）
- docs/phase-1/work-plans/2026-07-18_phase1a8-v2-work-plan.md（v3 → v3.1 升级）
- fincontrol-backend/src/main/java/com/fincontrol/{entity/AssetRaw,dto/{screenshot/ParsedAsset,asset/AssetBalanceItem,snapshot/SnapshotFundDetail},service/{ScreenshotService,SnapShotConfirmService,DedupEngine,AssetQueryService,SnapshotQueryService}}.java
- fincontrol-backend/src/test/java/com/fincontrol/fixture/Phase1a8RealFourPageFixture.java（holding+cumulative record）
- fincontrol-backend/src/test/java/com/fincontrol/service/Phase1a8RealFourPageFixtureTest.java
- fincontrol-backend/src/test/resources/fixtures/phase1a8-real-four-pages.json（v3.1 双字段）
- fincontrol-backend/src/test/resources/schema-h2.sql（+2 列）
- fincontrol-backend/scripts/1a8/01-real-four-page-e2e.ps1（Compare-FundSet holding + cumulative + per-page totalAsset 规则）

不入仓（ignored）：application-local.yml / uploads/ / ocr-results/ / api-test-output/ / pre-v3.1-failed
