...
**验收目标（DoD）**：
- [ ] 投资决策类问题 → main_loop（带 ai_assistant system prompt）
- [ ] 闲聊/技术支持/写作/模型身份询问 → garbage_loop（无 system prompt）
- [ ] 响应中 `routedTo` 字段与走过的回路一致
- [ ] conversation 列表 CRUD 全部走通
- [ ] chat_history 写入正确（role/user|assistant、content 不丢失）
